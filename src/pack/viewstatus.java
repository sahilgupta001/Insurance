	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			PreparedStatement ps=conn.prepareStatement
					("select verification from upload where id=?");
			ps.setInt(1,id);
			...
			while(r.next()) {
				...
				out.println("<td><a href='ViewDocument?customerId="+cid+"&&st=birth'>View Birth Certificate</a></td>");
				out.println("<td><a href='ViewDocument?customerId="+cid+"&&st=id'>View ID Proof</a></td>");
			...
			e.printStackTrace();
				...
				if(status.equals("Documents Verified"))
				{
					
					out.println("<form action='payment.jsp'>");
					out.println("<br/><br/>");
					out.println("<input type='submit' value='Buy Now' name='b1'>");
					out.println("</form>");
